"""Visualize trajectories of one or more env3 players on environment_3.

Replays the same RNN loop as challenge_3.evaluate() but records the bot
position at every step and overlays it on the sampled env3 world map.
Supports plotting multiple models side-by-side (rows) across multiple
seeds (columns).
"""
...
PLAYERS = {
    "random":     ("env3_player_random",     "random_player"),
    "wallfollow": ("env3_player_wallfollow", "wallfollow_player"),
    "bio":        ("env3_player_bio",        "bio_player"),
}
...
def plot_trajectory(traj, ax=None, title=None):
    ...
    ax.scatter(positions[:, 0], positions[:, 1], c=t, cmap="viridis",
               s=2, zorder=10)
    ax.plot(positions[0, 0], positions[0, 1], "o", color="lime", markersize=10,
            markeredgecolor="black", label="start", zorder=20)
    ax.plot(positions[-1, 0], positions[-1, 1], "X", color="red", markersize=10,
            markeredgecolor="black", label="end", zorder=20)
    if len(traj["hits"]):
        hits = positions[traj["hits"]]
        ax.scatter(hits[:, 0], hits[:, 1], facecolor="none", edgecolor="red",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot env3 trajectories for one or more player models.")
    parser.add_argument(
    ...
    parser.add_argument("--save", default=None,
                        help="Output image path. Default: "
                             "trajectory_env3_<model1>[_<model2>...].png")
...
if __name__ == "__main__":
    ...
    save_path = args.save or f"trajectory_env3_{'_'.join(models)}.png"
    plot_models(
        player_keys=models,
        seeds=args.seeds,
